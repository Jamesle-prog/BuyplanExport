"""Built-in UI translation seed rows: ``(english_key, zh_text, category, module)``.

Data only — loaded by ``UITranslationStore.seed_defaults()`` on first use and
counted on the Admin → Translations page.  Add new UI strings here (or via
that page); the store never overwrites a translation an admin has edited.
"""
