...
_CHANGELOG: list[dict] = [
    {
        ...
        "date": "2026-09-01",
        "entries": [
            {"type": "feat", "text": "**传真 PO（KL / MSG / TK EU / Infor Nexus）现在会存进历史，自动出现在 Order Summary 和 PO Tracker 里。** 以前这四种格式解析完只生成一个 Excel 下载，什么都不存——而这两个汇总视图是从数据库读的，所以传真 PO 从来进不去，没法和 GIII 主流程、Sky East 的单子放在一起看。现在它们走和其他 PO 完全相同的保存流程，一并获得重复检测、改版历史和冲突提示：同一份传真再传一次是「重复」，改过件数再传是「更新」，不会变成两条。"},
            {"type": "feat", "text": "**每条记录标明来源格式。** PO Tracker 的 Format 列会显示 kl / msg / tk_eu / infor_nexus，一眼看出这单是从哪种传真解析来的。解析不出 PO 号的会跳过（PO 号是记录的身份，空号会和其他无号单据撞在一起）；价格栏里的 \"?\" 和 \"UNCONFIRMED\" 存为空白而不是文字，以免被当成数字参与合计。"},
        ],
    },
    {
        "version": "2.145.0",
        "date": "2026-09-01",
        "entries": [
            {"type": "feat", "text": "**GIII 生产计划单现在也带款式照片了，和 Sky East 一样。** 每个款号的表页上方空白处放正面照（有背面照就一并放上），照片来自和 Sky East 完全相同的照片库——同一个款拍一次，出现在哪份 buy plan 上都有图。GIII 的两个入口（上传后自动下载、Reports 里点生成）都自动带上，不需要额外操作。"},
            {"type": "feat", "text": "**照片按比例缩放，不会被拉变形。** Sky East 的模板预留了固定大小的图框，照片是撑满整框的；生产计划单的空白区又宽又扁，同样撑满会把服装照压扁，所以这里改为等比例缩放放入，并按图片高度自动撑开那一行。找不到照片就照常出单，和以前完全一样。"},
        ],
    },
    {
        "version": "2.144.1",
        "date": "2026-09-01",
        "entries": [
            {"type": "fix", "text": "**\"保存一份到文件夹\" 不再覆盖已有的同名文件。** 输出文件夹通常是共享盘，上面还有别人的东西；重名时旧文件会被直接冲掉，没有提示也没法恢复。现在遇到重名会存成 _v2、_v3（和系统自己导出时的规则一致），提示信息里显示实际写出的文件名，加了版本号也一眼可见。"},
            {"type": "docs", "text": "**全量审计：确认程序不会改动共享盘上任何已有文件。** 逐一检查了代码里 198 处文件写入和全部删除操作：所有导出都在临时目录生成（9 处 out_dir 全部是临时目录）后以下载方式交付；删除只发生在系统自己的模板目录和提取图文件夹（且有符号链接逃逸防护）；写向用户文件夹的路径只剩两条——照片提取（v2.144.0 已停止写入）和\"保存一份\"（本次改为不覆盖）。"},
        ],
    },
    {
        "version": "2.144.0",
        "date": "2026-09-01",
        "entries": [
            {"type": "feat", "text": "**你的照片文件夹从此只读不写。** 从客户合同里提取的图，现在只存进系统自己的文件夹（data/extracted_images），一张都不会再写进你配置的照片库。之前每次上传都会把客户 Excel 里嵌的图以款号命名写进你的共享盘，覆盖手工整理的照片——文件名不变，所以完全看不出来。"},
            {"type": "fix", "text": "**原始照片文件夹永远优先，找不到时才用提取的图。** 之前的查找顺序有漏洞：提取文件夹里的 {款号}_front.png 会压过你照片库里的 {款号}.png，等于用客户的图盖掉了你的图。现在会把你的照片库整个找完（含带空格等手写变体）才去看提取文件夹——提取的图只用来补空缺，永远不会顶掉你的照片。"},
        ],
    },
    {
        "version": "2.143.1",
        "date": "2026-09-01",
        "entries": [
            {"type": "fix", "text": "**上传合同不再覆盖你照片库里已有的款式照片。** 这才是款式图一直不对的真正原因：每次上传 Sky East 合同，系统都会把客户 Excel 里嵌的那张图，以 {款号}_front.png 的名字写进你配置的照片文件夹——文件名不变，所以看不出被换过，但图已经是客户文件里的那张了。TP3267-3_4SLV 的照片就是这样被换成了一件无袖上衣，之后每份核料单都印错。现在已有的照片永远不会被覆盖；只有还没有照片的款号才会写入一张。"},
        ],
    },
    {
        "version": "2.143.0",
        "date": "2026-09-01",
        "entries": [
            {"type": "feat", "text": "**款号里的 \"/\" 不再被改写 — 文件写什么，系统就存什么、显示什么。** 前两天的做法（入库时把 \"/\" 改成 \"_\"）导致 buy plan 和核料单上印出的款号和客户文件不一致，按用户要求已推翻重做。现在 \"/\" 只在**内部匹配**时当作 \"_\"：面料对照、单耗、生产跟踪、裁剪计划的查找，以及各处搜索框，两种写法互相都能找到 — 但屏幕上和导出文件里永远是原样。之前被改成 \"_\" 的历史数据（59 行）已全部恢复为原写法。"},
            {"type": "fix", "text": "**同一款号两种写法不会再产生两条生产跟踪记录。** 用 \"/\" 保存过再用 \"_\" 保存（或反过来）会落到同一条记录上，屏幕上保留最先录入的那种写法。"},
        ],
    },
    {
        "version": "2.142.2",
