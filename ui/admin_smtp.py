...
def show_smtp_admin() -> None:
    st.subheader(f"📧 {t('Email (SMTP) Settings')}")
    st.caption(t(
    ...
    st.markdown(f"**{t('Quick setup — choose your mail provider:')}**")
    ...
    if active:
        ...
        if label == "Outlook / Hotmail":
            st.warning(
                t("⚠️ **Microsoft has disabled basic SMTP auth for most personal Outlook/Hotmail "
            ...
            st.info(
                t("**If you still want to try Outlook:**\n\n"
            ...
            st.success(
                t("**📨 SendGrid — 100 emails/day free**\n\n"
            ...
            st.success(
                t("**💌 Brevo — 300 emails/day free (best free tier)**\n\n"
            ...
            st.success(
                t("**⚡ Resend — 100 emails/day free, 3,000/month**\n\n"
    ...
    with st.form("smtp_form"):
        ...
        with c1:
            host = st.text_input(t("SMTP Host"), value=_host_default(),
                                 placeholder="smtp.sendgrid.net")
        with c2:
            port = st.number_input(t("Port"), min_value=1, max_value=65535,
        ...
        with c4:
            ...
            password = st.text_input(
                t("Password / API Key / App Password"),
                value="",
                type="password",
                placeholder=(t("•••••• (saved — leave blank to keep)")
                             if cur["password"] else t("API key / app password")),
            )

        sender = st.text_input(
            t("Sender (From address)"),
            value=cur["sender"],
            placeholder="PO Extractor <you@example.com>",
            help=t("How the From address appears. Defaults to Username when empty."),
        )
    ...
    # ── Test connection ───────────────────────────────────────────────────────
    st.divider()
    st.markdown(f"**{t('Test connection')}**")
    if not smtp_settings.is_configured():
        st.info(
            t("Fill in Host, Username and Password/API Key, then click **Save** before testing."),
            icon="ℹ️",
        )
        return
    ...
    st.success(
        f"✅ {t('Configured:')} `{configured['host']}:{configured['port']}` "
        f"{t('as')} `{configured['user'] or configured['sender']}`",
    )
    ...
    if "brevo" in configured["host"].lower() and (
        ...
        if not _eff_sender:
            _sender_desc = t("empty — neither Username nor Sender is set")
        else:
            _sender_desc = f"`{_eff_sender}` — {t('this is the SMTP login username, NOT a verified sender')}"
        st.warning(
            t("⚠️ **Brevo: Sender address is not set correctly.**") + "\n\n"
            + f"{t('Your current From address is')} {_sender_desc}. "
            + t("Brevo will accept the connection but "
    ...
    if clicked:
        try:
            with st.spinner(f"{t('Connecting to')} {configured['host']}…"):
                send_test_email(to)
            st.success(f"✅ {t('Test email delivered to')} **{to}**.")
