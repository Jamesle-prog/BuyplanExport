"""Release changelog data — newest first.

Each entry: ``{version, date, entries: [{type, text}]}``;
types: feat | fix | perf | refactor | security | docs.

Add a new version at the **top** of ``CHANGELOG`` with every ``APP_VERSION``
bump (CLAUDE.md rule 3).  Rendering lives in ``ui/changelog_view.py``.
"""
...
CHANGELOG: list[dict] = [
    {
        ...
        "date": "2026-09-05",
        "entries": [
            {"type": "refactor", "text": "**Codebase refactor, phase 7 — data out of code.** The 1 635 built-in UI translation rows moved from `ui_translation_store.py` (2 260 lines → 289) into `po_extractor/store/_ui_translation_seed.py`, and the 328 changelog versions moved from `ui/changelog_view.py` (2 660 lines → 88) into `ui/changelog_data.py`. Both are pure data modules; the store and the Releases tab import them under their old names, so nothing else changed. CLAUDE.md rule 3 now points at `ui/changelog_data.py`"},
            {"type": "docs", "text": "Whole-codebase refactor summary (v2.125.1 → v2.125.5): dead root scripts removed; one normalisation module; one Excel styling / template / sheet-name toolkit; one store base (construction, once-per-process schema, migrations, DataFrame helpers, actor); a parser sheet scaffold; one live-schema cache; shared Select-all picker and xlsx-bytes helper; data tables split out. 1477 tests pass, 0 failures, 62 tests added along the way"},
        ],
    },
    {
        "version": "2.125.4",
