...
def _render_full_fabric_table(source: str) -> None:
    ...
    if search.strip():
        ...
        # Styles are stored exactly as the file spelled them; matching treats
        # "/" as "_" on BOTH sides, so either spelling typed finds either
        # spelling stored (utils/style_norm).
        needle_st = needle.replace("/", "_")
        df = df[
            df["style"].astype(str).str.lower().str.replace("/", "_", regex=False)
              .str.contains(needle_st, na=False, regex=False)
