...
def _fabric_db_paginated_list(store, total_count: int) -> None:
    ...
    with nav_l:
        st.button(t("◀ Prev"), disabled=(cur <= 1), key="fabric_db_prev",
                  on_click=lambda: st.session_state.update(
                      {"fabric_db_page": max(1, cur - 1)}))
    with nav_mid:
        picked = st.number_input(
            t("Page (1 – {n})").format(n=n_pages),
    ...
    st.caption(t(
        "Showing **{start} – {end}** of **{total}** fabric(s) · page **{cur} / {pages}**"
    ).format(start=f"{start:,}", end=f"{end:,}", total=f"{total_count:,}",
             cur=cur, pages=n_pages))


def _fabric_db_detail_card(store) -> None:
    """Render the 'view full detail for a specific fabric' expander."""
    with st.expander(t("🔎 View full detail for a specific fabric")):
        qno = st.text_input(
            t("Enter 公司面料编号 exactly"),
        ...
        st.markdown(t("**综合标识 Key:** `{key}`").format(key=rec.get('display_key', '')))
        st.divider()
