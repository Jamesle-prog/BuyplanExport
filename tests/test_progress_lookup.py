...
# ---------------------------------------------------------------------------
# build_po_style_lookup — (客户PO, 款号) is a more reliable join key than
# (PC No., style, colour name), since the same style + English colour name
# can carry a different Chinese colour/code across different POs.
# ---------------------------------------------------------------------------

def test_build_po_style_lookup_distinguishes_same_style_and_color_by_po():
    """Same style, same English colour name, same PC No. -- but two
    different POs give two different Chinese colours. Colour-name matching
    can't tell these apart; PO + style can.
    """
    import tempfile, os
    from po_extractor.lookups import ProgressLookup

    tmpdir = tempfile.mkdtemp()
    filepath = os.path.join(tmpdir, "progress_po.xlsx")
    data = {
        "序号":        [1, 2],
        "合同号":      ["C001", "C002"],
        "所在PO":      ["HHPPC048", "HHPPC048"],
        "IMAGE":       ["", ""],
        "款式":        ["DR5124", "DR5124"],
        "颜色":        ["Dark Blue", "Dark Blue"],
        "主标颜色":    ["", ""],
        "中文颜色":    ["藏青", "深蓝"],
        "中文颜色代码": ["52#", "58#"],
        "PO离厂日期":  ["", ""],
        "数量":        [100, 100],
        "PO#":         ["PO2333438C", "PO9999999X"],
        "BRAND":       ["Anna Field", "Anna Field"],
        "FABRICDETAIL": ["", ""],
    }
    pd.DataFrame(data).to_excel(filepath, sheet_name="2026 Zalando", index=False)
    ...
    by_po = pl.build_po_style_lookup()
    assert by_po[("PO2333438C", "DR5124")].cn_color == "藏青"
    assert by_po[("PO2333438C", "DR5124")].color_code == "52#"
    assert by_po[("PO9999999X", "DR5124")].cn_color == "深蓝"
    assert by_po[("PO9999999X", "DR5124")].color_code == "58#"


def test_build_po_style_lookup_skips_rows_with_no_po_recorded():
    import tempfile, os
    from po_extractor.lookups import ProgressLookup

    tmpdir = tempfile.mkdtemp()
    filepath = os.path.join(tmpdir, "progress_no_po.xlsx")
    data = {
        "序号":        [1],
        "合同号":      ["C001"],
        "所在PO":      ["HHPPC048"],
    ...
    assert pl.build_po_style_lookup() == {}


def test_build_po_style_lookup_recombines_split_two_tone_colours():
    """A two-tone row (see _split_multi_color) yields two colour records
    sharing the same PO + style -- they must be recombined with " / ",
    not have one silently dropped by the (po, style) grouping.
    """
    import tempfile, os
    from po_extractor.lookups import ProgressLookup

    tmpdir = tempfile.mkdtemp()
    filepath = os.path.join(tmpdir, "progress_two_tone.xlsx")
    data = {
        "序号":        [1],
        "合同号":      ["26302-ZA7156"],
        "所在PO":      ["HHPPC048"],
        "IMAGE":       [""],
        "款式":        ["DR5009"],
        "颜色汇总\n英文 中文 色号 色卡本": ["Dark BlUE /White 藏青 52#/白色 3#"],
        "英文颜色":    ["Dark BlUE"],
        "中文颜色":    ["藏青"],
        "中文颜色代码": ["52#/白色 3#"],
        "主标颜色":    [""],
    ...
    match = pl.build_po_style_lookup()[("PO2338236C", "DR5009")]
    assert match.cn_color == "藏青 / 白色"
    assert match.color_code == "52# / 3#"
